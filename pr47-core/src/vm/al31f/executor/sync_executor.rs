...
use crate::vm::al31f::compiled::{CompiledProgram, CompiledFunction};
...
// TODO find a way to unify synchronous and asynchronous version
pub unsafe fn vm_run_function_sync<A: Alloc>(
    ...
    let mut vm: AL31F<A> = AL31F::new(alloc);
    ...
    vm.alloc.set_gc_allowed(true);

    let compiled_function: &CompiledFunction = &program.functions[func_ptr];
    ...
    loop {
        ...
        match insc {
            ...
            Insc::LoadConst(const_id, dst) => {
                let constant: Value = *program.const_pool.get_unchecked(*const_id);
                slice.set_value(*dst, constant);
            }
            Insc::SaveConst(const_src, const_id) => {
                let constant: Value = slice.get_value(*const_src);
                *program.const_pool.get_unchecked_mut(*const_id) = constant;
            }
            ...
            Insc::CreateObject(dest) => {
                ...
                vm.alloc.add_managed(object.ptr_repr);
            ...
            _ => unreachable!("asynchronous instructions not allowed in synchronous executor")
        }
    }
}
